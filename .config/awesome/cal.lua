...
local l_not_show_week_nr = false
...
function displayMonth(month,year,weekStart, not_show_week_nr)
...
function register(mywidget, custom_current_day_format, not_show_week_nr)
        l_not_show_week_nr = not_show_week_nr
	...
	if not tooltip then
		tooltip = awful.tooltip({})
                function tooltip:update()
                        ...
                        state = {month, year}
                        tooltip:set_text(string.format('<span font_desc="monospace">%s</span>', displayMonth(month, year, 2, l_not_show_week_nr)))
                end
                tooltip:update()
	end
...
function switchMonth(delta)
	state[1] = state[1] + (delta or 1)
        local text = string.format('<span font_desc="monospace">%s</span>', displayMonth(state[1], state[2], 2, l_not_show_week_nr))
